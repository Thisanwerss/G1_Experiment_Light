...
class HumanoidMocap(Task):
    ...
    def __init__(
        ...
        GROUND_BUFFER = 0.035  # Allow feet to be slightly above ground

        for i in range(n_frames):
            # === Apply reference pose ===
            qpos = self.reference[i].copy()
            mj_data.qpos[:] = qpos
            mujoco.mj_forward(mj_model, mj_data)

            # === Compute foot positions ===
            left_foot_z = mj_data.site_xpos[mj_model.site("left_foot").id][2]
            right_foot_z = mj_data.site_xpos[mj_model.site("right_foot").id][2]
            min_foot_z = min(left_foot_z, right_foot_z)

            # === Lower the base if needed to bring lowest foot to contact ===
            if min_foot_z > GROUND_BUFFER:
                correction = min_foot_z - GROUND_BUFFER
                qpos = qpos.at[2].add(-correction)
                mj_data.qpos[:] = qpos  # Re-apply updated qpos
                mujoco.mj_forward(mj_model, mj_data)

            # === Save corrected reference ===
            self.reference = self.reference.at[i].set(qpos)

            # === Store foot poses and orientations ===
            ref_left_foot_pos[i] = mj_data.site_xpos[mj_model.site("left_foot").id]
            ref_right_foot_pos[i] = mj_data.site_xpos[mj_model.site("right_foot").id]
            ...
            # === Store palm poses and orientations ===
            ref_left_palm_pos[i] = mj_data.site_xpos[mj_model.site("left_hand").id]
            ref_right_palm_pos[i] = mj_data.site_xpos[mj_model.site("right_hand").id]
    ...
    def running_cost(self, state: mjx.Data, control: jax.Array) -> jax.Array:
        ...
        # Action rate cost
        action_rate_cost = jnp.sum(jnp.square(control - q[7:]))

        return (
            1.0 * joint_cost
            + 1.0 * base_cost
            + 1.0 * foot_cost
            + 0.0 * action_rate_cost
        )
