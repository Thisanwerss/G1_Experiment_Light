...
def run_interactive(  # noqa: PLR0912, PLR0915
    ...
    # Prepare to store torques
    torque_log = []

    # Optional: store actuator names
    ...
    with mujoco.viewer.launch_passive(mj_model, mj_data) as viewer:
        ...
        # Initialize first control sequence
        mj_data.ctrl[:] = np.zeros((1, 41))
        ...
        while viewer.is_running():
            ...
            for i in range(sim_steps_per_replan):
                ...
                # Log actuator torques (copy to avoid reference issues)
                torque_log.append(np.copy(mj_data.actuator_force))
    ...
    # # Convert to array
    # torque_log = np.array(torque_log)  # shape: (timesteps, actuators)

    # # Plot all actuators
    # plt.figure(figsize=(15, 8))
    # for i in range(mj_model.nu):
    #     if "right_shoulder" in actuator_names[i]:
    #         plt.plot(torque_log[:, i], label=actuator_names[i])

    # plt.title("Joint Torques (Actuator Forces) Over Time")
    # plt.xlabel("Timestep")
