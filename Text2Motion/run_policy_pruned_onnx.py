#!/usr/bin/env python3
"""
Isolated control computation node - ONNX optimized version
Bidirectional asynchronous ZeroMQ to receive robot state and return PD control targets

Usage:
    python run_policy_pruned_onnx.py --onnx_model_path exported_models/model.onnx

Workflow:
1. Initialize ONNX model and CEM pipeline with JIT warmup
2. Start bidirectional asynchronous ZeroMQ service (PULL/PUSH mode)
3. Use cycle_id barrier to receive state -> ONNX prediction + CEM optimization + interpolation -> return control sequence
...
def predict_knots_onnx(ort_session, qpos: np.ndarray, qvel: np.ndarray) -> np.ndarray:
    """Predict knots using ONNX model"""
    # Concatenate state
    state = np.concatenate([qpos, qvel], axis=0).astype(np.float32)
    state = state.reshape(1, -1)  # Add batch dimension
    
    # ONNX inference
    outputs = ort_session.run(None, {'input': state})
    knots = outputs[0].reshape(4, 41)
...
class AsyncControllerServiceONNX:
    """ONNX-optimized asynchronous control computation service"""
    ...
    def __init__(
        ...
        """Initialize ONNX-optimized asynchronous control service"""
        self.device = device
        ...
        print(f"🚀 Initializing ONNX-optimized asynchronous control computation service")
        ...
        initial_knots = predict_knots_onnx(self.ort_session, self.mj_data.qpos, self.mj_data.qvel)
    ...
    def compute_controls(
        ...
        """Compute control outputs (ONNX-optimized hybrid logic)"""
        self.t_ctrl_compute_start = time.time()
        timing_info = {}
        
        # 1. ONNX neural network prediction
        nn_start = time.time()
        predicted_knots = predict_knots_onnx(self.ort_session, qpos, qvel)
    ...
    def print_periodic_stats(self):
        ...
        print(f"\n🔍 === ONNX-Optimized Control Statistics Report (Cycle #{self.cycle_id}) ===")
    ...
    def handle_async_requests(self):
        ...
        print("💡 Working mode:")
        print("   - ONNX-optimized neural network inference")
    ...
    def start(self):
        """Start service"""
        print("🚀 Starting ONNX-optimized asynchronous control computation service...")
        self.running = True
        
        # Warmup phase: run several dummy loops to ensure ONNX and JIT are fully warmed up
        print("🔥 Warming up ONNX and JIT...")
        ...
        print("✅ ONNX and JIT warmup complete!")
        print("✅ ONNX-optimized control computation pipeline started and running!")
        print("💡 System status:")
        print("   - ONNX model fully warmed up")
    ...
    def stop(self):
        ...
        print(f"\n🏁 === Final ONNX-Optimized Control Report ===")
        ...
        print("✅ ONNX-optimized control computation service stopped")


def main():
    parser = argparse.ArgumentParser(
        description="ONNX-optimized control computation node - Bidirectional asynchronous ZeroMQ hybrid control service"
    )
    parser.add_argument(
        "--onnx_model_path", 
        type=str, 
    ...
    parser.add_argument("--num_samples", type=int, default=85, help="CEM sample count (lower for speed)")
    ...
    # Create and start ONNX-optimized control service
    service = AsyncControllerServiceONNX(
